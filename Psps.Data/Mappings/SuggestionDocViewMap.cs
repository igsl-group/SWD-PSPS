...
{
    public partial class SuggestionDocViewMap : BaseEntityMap<SuggestionDocView, int>
    {
        ...
        protected override void MapEntity()
        {
            ...
            Map(x => x.SuggestionNature).Column("SuggestionNature").Length(200);
            Map(x => x.SuggestionActivityConcernOther).Column("SuggestionActivityConcernOther").Length(100);
            Map(x => x.SuggestionActivityConcern).Column("SuggestionActivityConcern").Length(200);
            Map(x => x.SuggestionSourceOther).Column("SuggestionSourceOther").Length(100);
            Map(x => x.SuggestionSource).Column("SuggestionSource").Length(200);
            Map(x => x.SuggestionRefNum).Column("SuggestionRefNum").Length(20);
            Map(x => x.SuggestionMasterId).Column("SuggestionMasterId").Not.Nullable().Precision(10);
        }
    }
}