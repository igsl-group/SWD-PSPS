...
namespace DocxGenerator.Library
{
    ...
    public class OpenXmlHelper
    {
        ...
        public void SetContentOfContentControl(SdtElement contentControl, string content)
        {
            ...
            // Try to get the Content CheckBox if exist (NULL if the content is not a Checkbox)
            DocumentFormat.OpenXml.Office2010.Word.SdtContentCheckBox SdtContentCheckBox =
                contentControl.SdtProperties.Elements<DocumentFormat.OpenXml.Office2010.Word.SdtContentCheckBox>().FirstOrDefault();
            ...
            else if (SdtContentCheckBox != null)
            {
                // Case for CheckBox Content

                OpenXmlCompositeElement openXmlCompositeElement = GetSdtContentOfContentControl(contentControl);
                Run run = null;

                // Check the input value (boolean) and set the corresponding value of the checkbox
                if (content.ToUpper() == "TRUE")
                {
                    SdtContentCheckBox.Checked.Val = DocumentFormat.OpenXml.Office2010.Word.OnOffValues.One;
                    //run = CreateRun(openXmlCompositeElement, "☒");
                    run = CreateRun(openXmlCompositeElement, "☑");                    
                }
                else
                {
                    SdtContentCheckBox.Checked.Val = DocumentFormat.OpenXml.Office2010.Word.OnOffValues.Zero;
                    run = CreateRun(openXmlCompositeElement, "☐");
                }
            ...
            else
            {
                ...
                // Replace the content , prevent insert duplicate break line
                content = content.Replace(Environment.NewLine, "\n");
                content = content.Replace("\r", "\n");

                if (IsContentControlMultiline(contentControl) || content.Contains("\n"))
                {
                    ...
                    {
                        Run run = CreateRun(openXmlCompositeElement, "");

                        // Start adding the break (newlinew) during the second item and the after
                        if (addBreak)
                        {
                            run.AppendChild<Break>(new Break());
                        }
                        // Start the flag at the first item
                        if (!addBreak)
                        {
                            addBreak = true;
                        }
                        ...
                        run.AppendChild(new Text(textSplit));
