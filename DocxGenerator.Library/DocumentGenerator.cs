...
namespace DocxGenerator.Library
{
    ...
    public abstract class DocumentGenerator
    {
        ...
        /// </summary>
        /// <param name="dataContextType">Type of data context</param>
        /// <param name="parentName">Parnet node name</param>
        /// <returns>Placeholder tag type collection</returns>
        protected abstract Dictionary<string, PlaceHolderType> GetPlaceHolderTagToTypeCollection(Type dataContextType, string parentName = null);
        ...
        protected void SetContentInPlaceholders(OpenXmlElementDataContext openXmlElementDataContext)
        {
            if (IsContentControl(openXmlElementDataContext))
            {
                ...
                var fqn = string.IsNullOrEmpty(openXmlElementDataContext.ParentTag) ? templateTagPart : openXmlElementDataContext.ParentTag + "." + templateTagPart;

                if (this.generationInfo.PlaceHolderTagToTypeCollection.ContainsKey(fqn))
                {
                    this.OnPlaceHolderFound(openXmlElementDataContext);
                }
                else if (string.IsNullOrEmpty(templateTagPart))
                {
                    this.PopulateOtherOpenXmlElements(openXmlElementDataContext);
                }
                else
                {
                    // Set the content to [Invalid Tag] if not found
                    this.SetContentOfContentControl(openXmlElementDataContext.Element as SdtElement, "Invalid Tag");
                    this.PopulateOtherOpenXmlElements(openXmlElementDataContext);
                }
            }
            else
        ...
        protected SdtElement CloneElementAndSetContentInPlaceholders(OpenXmlElementDataContext openXmlElementDataContext)
        {
            ...
            {
                this.SetContentInPlaceholders(new OpenXmlElementDataContext() { Element = v, DataContext = openXmlElementDataContext.DataContext, ParentTag = openXmlElementDataContext.ParentTag });
            }
        ...
        public byte[] GenerateDocument()
        {
            ...
            this.generationInfo.PlaceHolderTagToTypeCollection = this.GetPlaceHolderTagToTypeCollection(this.generationInfo.DataContext.GetType());
        ...
        private void PopulateOtherOpenXmlElements(OpenXmlElementDataContext openXmlElementDataContext)
        {
            if (openXmlElementDataContext.Element is OpenXmlCompositeElement && openXmlElementDataContext.Element.HasChildren)
            {
                ...
                foreach (var element in elements)
                {
                    ...
                    {
                        this.SetContentInPlaceholders(new OpenXmlElementDataContext() { Element = element, DataContext = openXmlElementDataContext.DataContext, ParentTag = openXmlElementDataContext.ParentTag });
                    }
                }
            }
        }
        ...
        private void OnPlaceHolderFound(OpenXmlElementDataContext openXmlElementDataContext)
        {
            ...
            if (this.generationInfo.PlaceHolderTagToTypeCollection.ContainsKey(fqn))
            {
                switch (this.generationInfo.PlaceHolderTagToTypeCollection[fqn])
                {
