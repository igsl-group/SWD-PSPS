...
namespace Psps.Web.Core.Autofac
{
    public static class AutofacExtensions
    {
        ...
        /// <summary>
        /// Intercept ALL registered interfaces with provided interceptors. Override Autofac
        /// activation with a Interface Proxy. Does not intercept classes, only interface bindings.
        /// </summary>
        ...
        {
            // proxy does not get allong well with Activated event and registrations with Activated
            // events cannot be proxied. They are casted to LimitedType in the IRegistrationBuilder
            // OnActivated method. This is the offending Autofac code:
            // 
            // public IRegistrationBuilder<TLimit, TActivatorData, TRegistrationStyle>
            // OnActivated(Action<IActivatedEventArgs<TLimit>> handler) { if (handler == null) throw
            // new ArgumentNullException("handler"); RegistrationData.ActivatedHandlers.Add( (s, e)
            // => handler(new ActivatedEventArgs<TLimit>(e.Context, e.Component, e.Parameters,
            // (TLimit)e.Instance))); return this; }
